...
(
	...
	*/
	...
	FROM
		SummarizedPlanCache SPC
	WHERE
		SPC.execution_count_rank <= @TopResults
		OR SPC.total_worker_time_rank <= @TopResults
		OR SPC.total_logical_reads_rank <= @TopResults
		OR SPC.total_logical_writes_rank <= @TopResults
		OR SPC.total_physical_reads_rank <= @TopResults
		OR SPC.total_elapsed_time_rank <= @TopResults
)
SELECT
